...
<table class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
        <form name="searchinfo" method="GET" class="am-form"  action="ListInfo.php">
    <tr>
            <td> 
...
<table class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
  <tr>
    <td<?=$indexchecked==1?' class="header"':' bgcolor="#C9F1FF"'?>><div align="center"><a href="ListInfo.php?mid=<?=$mid?>">已发布</a></div></td>
    <td<?=$indexchecked==0?' class="header"':' bgcolor="#C9F1FF"'?>><div align="center"><a href="ListInfo.php?mid=<?=$mid?>&ecmscheck=1">待审核</a></div></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
<table class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
  ...
  <tr id=news<?=$r[id]?>> 
    ...
    <td> <div align="center"><?=$newstime?></div></td>
	<td> <div align="center"> <a title="下载次数:<?=$r[totaldown]?>"> 
        <?=$r[onclick]?>
        </a> </div></td>
    ...
    <td><div align="center"><a href="AddInfo.php?enews=MEditInfo&classid=<?=$r[classid]?>&id=<?=$r[id]?>&mid=<?=$mid?><?=$addecmscheck?>">修改</a> | <a href="ecms.php?enews=MDelInfo&classid=<?=$r[classid]?>&id=<?=$r[id]?>&mid=<?=$mid?><?=$addecmscheck?>" onclick="return confirm('确认要删除?');">删除</a> 
      </div></td>
  </tr>
  <?
	}
